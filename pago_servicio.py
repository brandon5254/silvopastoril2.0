...
def procesar(name, document, n_document, mail, country, city, address, phone, institute, ocupation, f_pago, participation, ponencia, english, coments, social, ruc, precio):
    ...
    if f_pago == 'bancard' or 'procard':
        ...
        response = CrearPedido(token, ruc, mail, name, phone, n_document, social, public_key ,monto_total, descripcion, 
        "Inscripción X CONGRESO SILVOPASTORIL", public_key, monto_total, fecha_maxima_pago, id_pedido, descripcion_resumen)#crea el pedido
        all = json.loads(response)
        token_recibed = response['resultado'][0]['data']
        ...
        "Inscripción X CONGRESO SILVOPASTORIL", public_key, descripcion, 
        monto_total, fecha_maxima_pago, id_pedido, descripcion_resumen)#crea el pedido
        all = json.loads(response)
        token_recibed = response['resultado'][0]['data']
        dbHandler.updateByID(token_recibed, id_pedido)
        num = dict.get(f_pago)
